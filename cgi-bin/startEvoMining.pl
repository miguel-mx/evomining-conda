...
sub createOutDir{
	...
	if (!-d "../exchange/$output_path"){
		system("mkdir -p ../exchange/$output_path/blast/seqf/tree");
#		system ("chmod -R 777 /var/www/html/EvoMining/exchange/$output_path");
		print "mkdir EvoMining/exchange/$output_path has been created\n";
		}
	else {
		print "mkdir ../exchange/$output_path already exists\n";
#		system ("chmod -R 777 /var/www/html/EvoMining/exchange/$output_path");
		}
	print "Ready output directory \n\n";
	unless(-e "../exchange/$output_path/EvoMining.log"){
...
sub prepareDB{
	...
	my $rast_ids=shift;
	my $smash_db=shift;
	my $import_p="../EvoMining/exchange";
	my $moved_p="../EvoMining/cgi-bin";
